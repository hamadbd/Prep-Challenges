...
const recursionPattern = (int1, int2) => {
    ...
    var ax = [];
    var test = false;
    const recursionTwo = (int1, int2) => {
        if(ax.length > 1 && ax[0] == int1){
            ax.push(int1);
            return ;
        }
        if (int1 < 0) {
...
const filterLinks = (str) => {
    // write your code here
    let regx = /(www.)+[+\w]+(.com|.net|.org)/g;
    let webUrl = str.match(regx);
    return webUrl.join("");
}
// -------------------------------------------------------------------------------------------------------
...
const isPalindrome = (str) => {
    ...
    if (str === null || str === " ") {
        ...
        const lowering = str.toLowerCase();
        //to convert to small letters
        const regx = /\w/g;
        const first = lowering.match(regx);
        const firstX = first.join("");
        
        const second = (lowering.match(regx)).reverse();
        const secondX = second.join("");
